...
Đặt tên branch dạng `<loại>/<mo-ta-kebab-case>`, ví dụ: `feature/order-status-management`, `fix/category-cycle-check`.

## Pull Request

- Mọi thay đổi vào `develop`/`main` đều qua Pull Request, không push thẳng.
- PR cần mô tả ngắn gọn **why** (không chỉ liệt kê file đổi), và checklist đã chạy lint/build/test theo `CLAUDE.md`.
...
Ví dụ: `feat: add stock movement audit trail`.

## Quy tắc riêng cho schema Prisma

Repo này là nơi duy nhất chạy `prisma migrate dev` (xem README). Khi PR có thay đổi `prisma/schema.prisma`:
- Bắt buộc có migration mới trong `prisma/migrations/` kèm theo (không sửa tay migration cũ).
- Sau khi merge, nhớ đồng bộ `schema.prisma` + migration mới sang `backend-user` (chạy `prisma generate` bên đó).
