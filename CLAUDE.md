...
- Trước khi bắt tay vào tính năng mới (không áp dụng cho fix nhỏ đang dở): rà soát nhanh phần code liên quan ở nhánh làm việc hiện tại của **cả `backend-cms` lẫn `frontend-admin`** (2 repo luôn đi cùng nhau — CMS BE phục vụ trực tiếp FE Admin), tìm lỗi logic còn tồn đọng quanh khu vực tính năng sắp đụng tới (API trả sai field/status, thiếu validate, N+1 query, FE gọi sai contract Swagger hiện tại...). Có lỗi thì sửa trước — commit riêng, có thể trên nhánh `fix/...` tách biệt — rồi mới bắt đầu code tính năng mới, không dồn việc dọn nợ này tới lúc tính năng mới xong mới làm. Mục tiêu: code không bug, logic đúng, dễ maintain, không lặp code.
